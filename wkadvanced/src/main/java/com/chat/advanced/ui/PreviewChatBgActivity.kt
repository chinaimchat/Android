...
//                    .getInt(userInfoEntity.uid + "_" + WKChannelCustomerExtras.chatBgColorIndex)
            ...
            if (channel?.localExtra != null) {
                ...
                if (urlObject == url) {
                    ...
                    if (isSvgObject != null) {
                        (channel.localExtra[ChatBgKeys.chatBgColorLight] as? String)?.let { lightColors = it }
                        (channel.localExtra[ChatBgKeys.chatBgColorDark] as? String)?.let { darkColors = it }
                        gradientAngle = (channel.localExtra[ChatBgKeys.chatBgGradientAngle] as? Number)?.toInt() ?: 0
                        (channel.localExtra[ChatBgKeys.chatBgShowPattern] as? Number)?.toInt()?.let { showPattern ->
                            wkVBinding.patternCB.setChecked(showPattern == 1, true)
                        }
                    }
                    (isBlurredObject as? Number)?.toInt()?.let { isBlurred ->
                        wkVBinding.blurredCB.setChecked(isBlurred == 1, true)
                        wkVBinding.blurView.visibility = if (isBlurred == 1) View.VISIBLE else wkVBinding.blurView.visibility
                    }
                }
            }
        }
