...
public class EmojiFragment extends WKBaseFragment<FragEmojiLayoutBinding> {
    ...
    protected void initView() {
        ...
        for (String s : EmojiManager.getInstance().getEmojiWithType("0_")) {
            emojiIndexs.add(new EmojiEntry("", s, ""));
        }
        for (String s : EmojiManager.getInstance().getEmojiWithType("1_")) {
            emojiIndexs.add(new EmojiEntry("", s, ""));
        }
        for (String s : EmojiManager.getInstance().getEmojiWithType("2_")) {
            emojiIndexs.add(new EmojiEntry("", s, ""));
        }
    ...
    protected void initListener() {
        emojiAdapter.setOnItemClickListener((adapter, view, position) -> {
            EmojiEntry entry = (EmojiEntry) adapter.getItem(position);
