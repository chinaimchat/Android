...
// 在ImageView上显示圆角头像
    ...
    public void showAvatarImg(Context mContext, String url, String key, ImageView imageView) {
        if (mContext != null) {
            ...
            if (context instanceof Activity activity) {
                if (!activity.isDestroyed()) {
                    String finalUrl = WKApiConfig.appendAvatarCacheKey(url, key);
                    RequestOptions avatarOptions = GlideRequestOptions.getInstance().normalRequestOption();
                    Drawable currentDrawable = imageView.getDrawable();
                    if (currentDrawable != null) {
                        // Avoid visible flashing when the same avatar is loaded repeatedly.
                        avatarOptions = avatarOptions.placeholder(currentDrawable);
                    }
                    Glide.with(context).load(finalUrl).dontAnimate()
                            .apply(avatarOptions)
                            .into(imageView);
                }
            }
