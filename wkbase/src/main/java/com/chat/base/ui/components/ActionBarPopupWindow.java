...
public class ActionBarPopupWindow extends PopupWindow {
    ...
    /** Anchor and offset for scroll-driven repositioning (no reflection on PopupWindow). */
    private View mScrollAnchor;
    private int mScrollXoff;
    private int mScrollYoff;
    private final ViewTreeObserver.OnScrollChangedListener mOurScrollListener = () -> {
        if (mScrollAnchor != null && isShowing()) {
            update(mScrollAnchor, mScrollXoff, mScrollYoff, getWidth(), getHeight());
        }
    };
...
//                            }
    ...
    private void init() {
        // No reflection: scroll-based repositioning is done via mOurScrollListener on the anchor.
    }
    ...
    private void unregisterListener() {
        if (mViewTreeObserver != null && mViewTreeObserver.isAlive()) {
            mViewTreeObserver.removeOnScrollChangedListener(mOurScrollListener);
        }
    ...
    private void registerListener(View anchor) {
        ...
        if (vto != mViewTreeObserver) {
            ...
            mViewTreeObserver = vto;
            vto.addOnScrollChangedListener(mOurScrollListener);
        }
    }
