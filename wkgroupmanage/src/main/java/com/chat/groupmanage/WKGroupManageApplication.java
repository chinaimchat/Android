...
public class WKGroupManageApplication {
    ...
    private View getGroupInUserDetailView(Context context, String uid, String groupNo, ViewGroup parentView) {
        ...
        final WKChannelMember[] member = {WKIM.getInstance().getChannelMembersManager().getMember(groupNo, WKChannelType.GROUP, uid)};

        if (member[0] != null) {
            if (member[0].forbiddenExpirationTime > 0) {
                timeTv.setText(R.string.forbiddening);
                setTime(member[0].forbiddenExpirationTime, context, contentTv);
            } else {
        ...
        SingleClickUtil.onSingleClick(view.findViewById(R.id.forbiddenLayout), view1 -> {
            ...
            if (member[0] != null && member[0].forbiddenExpirationTime == 0) {
