...
public class GroupHeaderActivity extends WKBaseActivity<ActGroupHeaderLayoutBinding> {
    ...
    protected void initView() {
        wkVBinding.getRoot().setPadding(0, 0, 0, 0);
        wkVBinding.avatarIv.post(() -> {
            int w = wkVBinding.avatarIv.getWidth();
            if (w > 0) {
                wkVBinding.avatarIv.getLayoutParams().height = w;
                wkVBinding.avatarIv.requestLayout();
            }
            wkVBinding.avatarIv.post(this::loadAvatarImage);
        });
    }
    ...
    private void loadAvatarImage() {
        ...
        String key = channel != null ? channel.avatarCacheKey : "";
