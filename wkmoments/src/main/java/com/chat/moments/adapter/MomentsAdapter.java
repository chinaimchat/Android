        ...
        if (moments.itemType > 0) {
            ...
            switch (moments.itemType) {
                ...
                case MomentsType.video_text: {
                    ...
                    if (!TextUtils.isEmpty(moments.video_cover_path)) {
                        // 如果后端带了宽高信息（xxx@宽x高.png），则根据比例调整控件大小
                        if (moments.video_cover_path.contains("@") && moments.video_cover_path.contains("x")) {
