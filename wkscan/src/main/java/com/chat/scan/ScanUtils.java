    ...
    void handleScanResult(AppCompatActivity activity, String result, @NonNull final IHandleScanResult iHandleScanResult) {
        ...
        try {
            if (result.startsWith("HTTP") || result.startsWith("http") || result.startsWith("www") || result.startsWith("WWW")) {
                ...
                // Accept qrcode path across different hosts (e.g. web domain vs api domain).
                if (resultURL.getPath() != null && resultURL.getPath().contains(baseURL.getPath())) {
                ...
                payload.put("uri", result);
                Object handled = EndpointManager.getInstance().invoke("wk_scan_handle_mtp", payload);
