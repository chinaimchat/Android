...
//            initStickerGridData();
        ...
        // 强制开启表情模块：忽略 app_module 中的 sticker 开关

        val stickerDir = resolveStickerDir("stickerCache")
        WKFileUtils.getInstance().createFileDir(stickerDir)
        stickerDirPath = resolveStickerDir("wkStickers")
...
//        initStickerGridData();
        ...
        EndpointManager.getInstance().setMethod("initInputPanel") { `object` ->
            ...
            val panelMenu = initPanelMenu ?: return@setMethod null
            val conversationContext = panelMenu.iConversationContext.get() ?: return@setMethod null
        ...
        EndpointManager.getInstance().setMethod("search_chat_edit_content") { `object` ->
            ...
            val currentMenu = searchStickerMenu
            val keyword = currentMenu?.content.orEmpty()
                ...
                StickerModel().search(
                    ...
                    object : StickerModel.ISearchListener {
                        override fun onResult(code: Int, msg: String, list: List<Sticker>) {
                            if (searchStickerMenu?.content != keyword) {
                                return
                            }
                            val localAdapter = adapter ?: return
                            if (list.isNotEmpty()) {
                                localAdapter.setList(ArrayList())
                                if (page == 1) {
                                    localAdapter.setList(list)
                                } else {
                                    localAdapter.addData(list)
                                }
                                resetData()
                                if (searchStickerRecyclerView?.visibility == GONE) {
                                    ...
                                    localAdapter.headerLayout?.layoutParams?.height =
                                        AndroidUtilities.getScreenHeight() - h
                                    localAdapter.headerLayout?.layoutParams?.height?.toFloat()
                                        ?.let { searchStickerRecyclerView?.setHeaderViewY(it) }
                                }
                            } else {
                                localAdapter.setList(ArrayList())
                            }
    ...
    private fun initSearchStickerView(
        ...
        val recyclerView = searchStickerRecyclerView ?: return
        recyclerView.layoutManager = FullyGridLayoutManager(
            iConversationContext.chatActivity, 5
        )
        recyclerView.setView(bottomView, headerView)
        recyclerView.addOnScrollListener(recyclerView.onScrollListener)
        adapter = SearchStickerAdapter()
        val localAdapter = adapter ?: return
        recyclerView.adapter = localAdapter
        localAdapter.addHeaderView(headerView)
        ...
        localAdapter.headerLayout?.layoutParams?.height =
            AndroidUtilities.getScreenHeight() - h
        localAdapter.headerLayout?.layoutParams?.height?.toFloat()
            ?.let { recyclerView.setHeaderViewY(it) }

        recyclerView.visibility = GONE
        localAdapter.addChildClickViewIds(R.id.imageView, R.id.stickerView)
        localAdapter.setOnItemClickListener { _, v, position ->
            val searchSticker = localAdapter.data.getOrNull(position) ?: return@setOnItemClickListener
    ...
    fun resetData() {
        val localAdapter = adapter ?: return
        val num = localAdapter.data.size % 5
        if (num != 0) {
            var count = 5 - num
            while (count > 0) {
                val sticker = Sticker()
                sticker.isNull = true
                localAdapter.addData(sticker)
                count--
            }
        }
    }

    private fun resolveStickerDir(dirName: String): String {
        val context = WKBaseApplication.getInstance().context
...
//    }
    ...
    private fun fetchSticker() {
        ...
        val isRefreshStickerCategory =
            WKSharedPreferencesUtil.getInstance().getBoolean("isRefreshStickerCategory")

        if (isRefreshStickerCategory && !TextUtils.isEmpty(WKConfig.getInstance().token)) {
        ...
        val isRefreshCustomerStickerCategory =
            WKSharedPreferencesUtil.getInstance().getBoolean("isRefreshCustomerStickerCategory")
        if (isRefreshCustomerStickerCategory && !TextUtils.isEmpty(WKConfig.getInstance().token)) {
