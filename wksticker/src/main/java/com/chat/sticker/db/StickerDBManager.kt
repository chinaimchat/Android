...
//            val num = getCustomStickerMaxSortNum()
    ...
    fun getEmojiSticker(searchable_word: String): Sticker {
        try {
            var sticker: Sticker? = null
            if (emojiStickerList.size > 0) {
                for (e in emojiStickerList) {
                    if (e.searchable_word == searchable_word) {
                        sticker = e
                        break
                    }
                }
            ...
            sticker = queryEmojiSticker("emoji", searchable_word)
