...
public class EmojiView extends FrameLayout {
    ...
    public EmojiView(@NonNull Context context) {
        ...
        EmojiAdapter adapter = new EmojiAdapter(AndroidUtilities.getScreenWidth());
        ...
        if (!TextUtils.isEmpty(ids)) {
            if (ids.contains(",")) {
                ...
                for (String id : idArr) {
                    if (list.size() == 24)
                        continue;
                    list.add(new EmojiEntity(id, 0));
                }
            } else {
                list.add(new EmojiEntity(ids, 0));
            }
            list.add(0, new EmojiEntity(context.getString(R.string.common_emoji), 1));
            list.add(new EmojiEntity(context.getString(R.string.all_emoji), 1));
        }

        List<String> normalList = EmojiManager.getInstance().getEmojiWithType("0_");
        for (String string : normalList) {
            list.add(new EmojiEntity(string, 0));
        }
...
//        updateStickerTabsPosition();
    ...
    private void emojiClick(EmojiEntity emojiEntity) {
        ...
        if (!TextUtils.isEmpty(usedIndexList)) {
            if (usedIndexList.contains(",")) {
                ...
                for (String string : strings) {
                    if (!TextUtils.isEmpty(string) && !string.contains(emojiEntity.getText())) {
        ...
        tempIndexList = emojiEntity.getText() + "," + tempIndexList;
