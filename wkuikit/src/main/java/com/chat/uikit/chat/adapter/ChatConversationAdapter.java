...
//            showContent(baseViewHolder, item);
    ...
    private String getContent(WKMsg msg) {
        ...
        if (msg.type == WKContentType.WK_WALLET_BALANCE_SYNC) {
            return getContext().getString(com.chat.base.R.string.wallet_balance_sync_session_preview);
        }
    ...
    private void showContent(@NotNull BaseViewHolder helper, WKUIConversationMsg item) {
        ...
        EmojiTextView contentTv = helper.getView(R.id.contentTv);
        ...
        applyConversationSubtitleColor(contentTv, item.getWkMsg(), content);
    }

    /**
     * 会话列表摘要：红包/转账等用与标题色接近的深灰（仅比 colorDark 弱约 20%），避免整行 #999 过浅；
     * 其它消息仍用 color999。
     */
    private void applyConversationSubtitleColor(EmojiTextView contentTv, WKMsg msg, String lineText) {
        if (contentTv == null) {
            return;
        }
        int baseDark = ContextCompat.getColor(getContext(), R.color.colorDark);
        int emphasis = ColorUtils.blendARGB(baseDark, Color.WHITE, 0.2f);
        int defaultMuted = ContextCompat.getColor(getContext(), R.color.color999);
        contentTv.setTextColor(shouldUseStrongerConversationSubtitle(msg, lineText) ? emphasis : defaultMuted);
    }

    private boolean shouldUseStrongerConversationSubtitle(@Nullable WKMsg msg, @Nullable String lineText) {
        String s = lineText != null ? lineText : "";
        if (msg != null) {
            int t = msg.type;
            if (t == WKContentType.WK_REDPACKET || t == WKContentType.WK_TRANSFER) {
                return true;
            }
            if (WKContentType.isSystemMsg(t)) {
                if (s.contains("领取") && s.contains("红包")) {
                    return true;
                }
                if (s.contains("[转账]")) {
                    return true;
                }
                if (s.contains("转账") && (s.contains("待确认") || s.contains("已收款") || s.contains("退回"))) {
                    return true;
                }
            }
        }
        return (s.contains("领取") && s.contains("红包"))
                || s.contains("[转账]")
                || (s.contains("转账") && (s.contains("待确认") || s.contains("已收款") || s.contains("退回")));
    }
    ...
    private void showReminders(@NotNull BaseViewHolder helper, ChatConversationMsg item) {
        ...
        if (!TextUtils.isEmpty(draft)) {
            ...
            if (contentTv instanceof EmojiTextView) {
                applyConversationSubtitleColor((EmojiTextView) contentTv, item.uiConversationMsg.getWkMsg(), draft);
            }
        } else {
...
//            if (!isScrolling)
    ...
    private void showTyping(@NotNull BaseViewHolder helper, ChatConversationMsg item) {
        ...
        if (item.typingStartTime > 0) {
            ...
            TextView typingContentTv = helper.getView(R.id.contentTv);
            helper.setText(R.id.contentTv, content);
            typingContentTv.setTextColor(ContextCompat.getColor(getContext(), R.color.color999));
        }
