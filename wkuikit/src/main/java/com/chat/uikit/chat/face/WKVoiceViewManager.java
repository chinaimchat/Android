    ...
    public View getVoiceView(IConversationContext iConversationContext) {
        ...
        recordAudioView.setRecordAudioListener(new RecordAudioView.IRecordAudioListener() {
            ...
            public boolean onRecordStop() {
                ...
                // Use wall-clock duration to avoid Timer/Handler drift.
                long durationMs = recordStartTime > 0 ? (System.currentTimeMillis() - recordStartTime) : recordTotalTime;
                int time = (int) (durationMs / 1000);
                if (time <= 0 || durationMs < minRecordTime) {
                    timer.cancel();
                    onRecordCancel();
                    return false;
                }
                ...
                // Ensure attachment upload uses localPath (used by WKSendMsgUtils.uploadChatAttachment)
                audioMsgModel.localPath = audioFileName;
