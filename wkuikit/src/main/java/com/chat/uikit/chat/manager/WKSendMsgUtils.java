...
public class WKSendMsgUtils {
    ...
    void uploadChatAttachment(WKMsg msg, IUploadAttacResultListener listener) {
        ...
        if (msg.type == WKContentType.WK_IMAGE || msg.type == WKContentType.WK_GIF || msg.type == WKContentType.WK_VOICE || msg.type == WKContentType.WK_LOCATION || msg.type == WKContentType.WK_FILE) {
            ...
            Runnable uploadVideo = () -> {
                if (!TextUtils.isEmpty(videoMsgModel.url)) {
                    listener.onUploadResult(true, videoMsgModel);
                    return;
                }
                WKUploader.getInstance().getUploadFileUrl(msg.channelID, msg.channelType, videoMsgModel.localPath, (url1, fileUrl) -> {
                    if (!TextUtils.isEmpty(url1)) {
                        WKUploader.getInstance().upload(url1, videoMsgModel.localPath, msg.clientSeq, new WKUploader.IUploadBack() {
                            @Override
                            public void onSuccess(String url2) {
                                videoMsgModel.url = url2;
                                listener.onUploadResult(true, videoMsgModel);
                            }
            ...
            if (TextUtils.isEmpty(videoMsgModel.cover) && !TextUtils.isEmpty(videoMsgModel.coverLocalPath)) {
