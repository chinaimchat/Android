...
class WKImageProvider : WKChatBaseProvider() {
    companion object {
        private val downloadingImages = ConcurrentHashMap<String, Boolean>()
    }
    ...
    override fun setData(
        ...
        if (uiChatMsgItemEntity.wkMsg.flame != 1) {
            cacheImageIfNeeded(uiChatMsgItemEntity)
        }
...
//        }
    ...
    private fun cacheImageIfNeeded(uiChatMsgItemEntity: WKUIChatMsgItemEntity) {
        val msg = uiChatMsgItemEntity.wkMsg ?: return
        val imgContent = msg.baseContentMsgModel as? WKImageContent ?: return
        if (!TextUtils.isEmpty(imgContent.localPath)) {
            val file = File(imgContent.localPath)
            if (file.exists() && file.length() > 0L) return
        }
        if (TextUtils.isEmpty(imgContent.url)) return
        val showUrl = WKApiConfig.getShowUrl(imgContent.url)
        if (TextUtils.isEmpty(showUrl)) return
        val cacheKey = msg.clientMsgNO
        if (downloadingImages.containsKey(cacheKey)) return
        downloadingImages[cacheKey] = true

        val ext = if (imgContent.url.contains(".")) {
            imgContent.url.substring(imgContent.url.lastIndexOf("."))
                .takeIf { it.length in 2..6 } ?: ".jpg"
        } else ".jpg"
        val fileDir = WKConstants.imageDir + msg.channelType + "/" + msg.channelID + "/"
        WKFileUtils.getInstance().createFileDir(fileDir)
        val filePath = fileDir + msg.clientMsgNO + ext

        WKDownloader.instance.download(showUrl, filePath, object : WKProgressManager.IProgress {
            override fun onProgress(tag: Any?, progress: Int) {}
            override fun onSuccess(tag: Any?, path: String?) {
                downloadingImages.remove(cacheKey)
                imgContent.localPath = filePath
                WKIM.getInstance().msgManager.updateContentAndRefresh(
                    msg.clientMsgNO, imgContent, false
                )
            }
            override fun onFail(tag: Any?, failMsg: String?) {
                downloadingImages.remove(cacheKey)
            }
        })
    }
