...
class FriendAdapter :
    ...
    override fun convert(holder: BaseViewHolder, item: FriendUIEntity) {
        ...
        if (item.channel.online == 1) {
            ...
            if (UserOnlineStatus.showPeerDeviceBreakdown(
                    item.channel.deviceFlag,
                    WKConfig.getInstance().appConfig
                )
            ) {
            ...
            val content = if (device.isNullOrEmpty()) context.getString(R.string.online) else String.format(
                "%s%s",
                device,
