...
public class ChatFragment extends WKBaseFragment<FragChatConversationLayoutBinding> {
    ...
    private static final String CHAT_HOME_TITLE = "AI抖商学院";
    ...
    protected void initView() {
        ...
        wkVBinding.textSwitcher.setText(CHAT_HOME_TITLE);
        //去除刷新条目闪动动画
        ...
        WKIM.getInstance().getMsgManager().addOnRefreshMsgListener("chat_fragment", (msg, left) -> {
            ...
            for (int i = 0, size = chatConversationAdapter.getData().size(); i < size; i++) {
                if (chatConversationAdapter.getData().get(i).uiConversationMsg.channelID.equals(msg.channelID) && chatConversationAdapter.getData().get(i).uiConversationMsg.channelType == msg.channelType && chatConversationAdapter.getData().get(i).uiConversationMsg.getWkMsg() != null && (chatConversationAdapter.getData().get(i).uiConversationMsg.getWkMsg().clientSeq == msg.clientSeq || chatConversationAdapter.getData().get(i).uiConversationMsg.getWkMsg().clientMsgNO.equals(msg.clientMsgNO))) {
                    WKMsg localMsg = chatConversationAdapter.getData().get(i).uiConversationMsg.getWkMsg();
                    String prevContent = localMsg.content;
                    WKMessageContent prevModel = localMsg.baseContentMsgModel;
                    String prevDisplay = safeDisplayContent(prevModel);
                    String nextDisplay = safeDisplayContent(msg.baseContentMsgModel);
                    if (!TextUtils.equals(prevContent, msg.content)
                            || prevModel != msg.baseContentMsgModel
                            || !TextUtils.equals(prevDisplay, nextDisplay)) {
                        chatConversationAdapter.getData().get(i).isResetContent = true;
                    }
                    localMsg.content = msg.content;
                    localMsg.baseContentMsgModel = msg.baseContentMsgModel;
...
//        return list;
    ...
    private void sortMsg(List<ChatConversationMsg> list) {
        ...
        AndroidUtilities.runOnUIThread(() -> {
            if (!isUiReady() || chatConversationAdapter == null) {
                return;
            }
...
//                                    chatConversationAdapter.notifyItemChanged(i, chatConversationAdapter.getData().get(i));
    ...
    public void onAttach(@NotNull Context context) {
        ...
        tabActivity = context instanceof TabActivity ? (TabActivity) context : null;
    }
...
//        } else pcLoginTv.setText(appLoginType);
        ...
        connectTimer.schedule(new TimerTask() {
            ...
            public void run() {
                ...
                if (context != null && isAdded()) {
                    EndpointManager.getInstance().invoke("show_disconnect_screen", context);
                }
            }
        }, 1000);
    }
    ...
    private boolean isUiReady() {
        return isAdded() && wkVBinding != null;
    }
    ...
    private void notifyRecycler(int index, ChatConversationMsg msg) {
        if (!isUiReady() || chatConversationAdapter == null || index < 0 || index >= chatConversationAdapter.getData().size()) {
            return;
        }
    ...
    private static String safeDisplayContent(WKMessageContent model) {
        if (model == null) {
            return "";
        }
        try {
            String s = model.getDisplayContent();
            return s != null ? s : "";
