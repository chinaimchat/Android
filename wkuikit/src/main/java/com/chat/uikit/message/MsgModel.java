...
public class MsgModel extends WKBaseModel {
   ...
   public List<WKChannelState> channelStatus;
    ...
    private WKSyncMsg getWKSyncMsg(SyncMsg syncMsg) {
        ...
        // Fallback: system welcome messages should still show unread
        // when backend omits red_dot in message header.
        if (WKSyncMsg.red_dot == 0
                && WKSystemAccount.system_team.equals(msg.fromUID)
                && msg.type != WKContentType.WK_INSIDE_MSG) {
            WKSyncMsg.red_dot = 1;
        }
        return WKSyncMsg;
    }
