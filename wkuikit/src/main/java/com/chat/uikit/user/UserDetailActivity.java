...
public class UserDetailActivity extends WKBaseActivity<ActUserDetailLayoutBinding> {
    ...
    private void bindJoinGroupWay(UserInfo userInfo) {
        wkVBinding.joinGroupWayLayout.setVisibility(View.GONE);
        if (userInfo == null) {
            return;
        }
        String inviteUid = userInfo.join_group_invite_uid;
        String inviteName = userInfo.join_group_invite_name;
        String joinTime = userInfo.join_group_time;
        if ((TextUtils.isEmpty(inviteUid) || TextUtils.isEmpty(inviteName) || TextUtils.isEmpty(joinTime))
                && userInfo.group_member != null) {
            inviteUid = userInfo.group_member.invite_uid;
            joinTime = userInfo.group_member.created_at;
        }
        if (TextUtils.isEmpty(inviteUid) || TextUtils.isEmpty(joinTime)) {
            return;
        }
        if (TextUtils.isEmpty(inviteName)) {
            return;
        }
        String content = String.format("%s %s", joinTime, String.format(getString(R.string.invite_join_group), inviteName));
        wkVBinding.joinGroupWayLayout.setVisibility(View.VISIBLE);
        wkVBinding.joinGroupWayTv.setText(content);
        int index = content.indexOf(inviteName);
        if (index < 0) {
            return;
        }
        SpannableString span = new SpannableString(content);
        span.setSpan(new NormalClickableSpan(false, Theme.colorAccount,
                new NormalClickableContent(NormalClickableContent.NormalClickableTypes.Other, ""), view -> {
                }), index, index + inviteName.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
